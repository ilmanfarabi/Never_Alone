    ...
    <>
      <header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between z-20 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline-block px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold border border-red-200 transition-colors shadow-xs group"
          >
            ...
            {notificationsOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 space-y-3 z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  <span className="font-bold text-xs text-slate-900">Urgent Safety & Action Queue</span>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {activeSOSCount > 0 && (
                    ...
                    >
                      ...
                      <div>
                        <div className="text-xs font-bold text-red-900">{activeSOSCount} Active SOS Distress Pings</div>
                        <div className="text-[11px] text-red-700">Immediate companion location tracking required.</div>
                      </div>
                    </div>
                  )}
                  ...
                  {tickets.filter(t => t.status === 'open').length > 0 && (
                    <div className="p-2.5 rounded-xl bg-blue-50 border border-blue-200 text-xs text-blue-900 font-medium">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-xs font-bold text-slate-800 transition-colors"
            >
          ...
          <div className="relative">
            ...
            >
              <div className="w-7 h-7 rounded-lg bg-[#1B3A4B] text-white flex items-center justify-center text-xs font-bold shadow-xs">
            ...
            {userMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border border-slate-200 p-3 space-y-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="p-2 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="text-xs font-bold text-slate-900">{currentAdmin?.name || 'Staff User'}</div>
                  <div className="text-[11px] text-slate-500 truncate">{currentAdmin?.email}</div>
                  <div className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-[#1B3A4B] uppercase bg-white px-2 py-0.5 rounded border border-slate-200">
          ...
          <button
            onClick={onExitAdmin}
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#FF6F61]/10 hover:bg-[#FF6F61]/20 text-[#FF6F61] text-xs font-bold transition-colors"
