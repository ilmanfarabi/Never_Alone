  ...
  const ledgerColumns: Column<TransactionLedgerItem>[] = [
    {
      ...
      cell: (tx: TransactionLedgerItem) => (
        <span className="font-mono text-xs font-semibold text-slate-800">{tx.id}</span>
      )
    },
    {
      header: 'Date & Time',
    ...
    },
    {
      ...
      cell: (tx: TransactionLedgerItem) => (
        <span className="font-mono text-xs text-indigo-600 hover:underline">{tx.bookingId}</span>
      )
    },
    {
      header: 'Customer',
      ...
      sortable: true,
      cell: (tx: TransactionLedgerItem) => (
        <span className="text-xs font-medium text-slate-900">{tx.customerName}</span>
      )
    },
    {
      header: 'Companion',
      ...
      sortable: true,
      cell: (tx: TransactionLedgerItem) => (
        <span className="text-xs text-slate-700">{tx.companionName}</span>
      )
    },
    {
      header: 'Gross Total',
      ...
      sortable: true,
      cell: (tx: TransactionLedgerItem) => (
        <span className="text-xs font-bold text-slate-900">৳{tx.totalAmount.toLocaleString()}</span>
      )
    },
    {
      ...
      cell: (tx: TransactionLedgerItem) => (
        <span className="text-xs text-emerald-600 font-medium">৳{tx.commissionAmount.toLocaleString()}</span>
      )
    },
    {
      ...
      cell: (tx: TransactionLedgerItem) => (
        <span className="text-xs text-slate-600">৳{tx.companionAmount.toLocaleString()}</span>
      )
    },
    {
      header: 'Method',
        ...
        <div className="flex items-center space-x-2">
          {(!tx.refundedAmount || tx.refundedAmount < tx.totalAmount) && (
            <button
              ...
              className="px-2 py-1 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded border border-amber-200 transition"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500">Total Gross Processed</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500">Net Platform Revenue (15%)</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-slate-500">Pending Companion Payouts</p>
      ...
      {activeTab === 'ledger' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-900">Complete Transaction Ledger</h2>
              <p className="text-xs text-slate-500">Every customer payment, escrow release, and refund record.</p>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-100">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Batch Companion Payout Queue</h2>
            ...
            {companionsWithPendingPayouts.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <h3 className="text-base font-bold text-slate-900">All Companion Payouts Cleared!</h3>
                      ...
                      <tr key={companion.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-slate-400 font-mono">{companion.id}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3.5 px-4">
                          <p className="text-slate-800">{companion.phone}</p>
                          <p className="text-[11px] text-slate-400">{companion.city}</p>
                        </td>
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center space-x-1.5">
                            ...
                            <span className="text-[11px] text-slate-500 font-mono">{companion.phone}</span>
                          </div>
                        </td>
      ...
      {activeTab === 'commission' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 max-w-2xl">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">Platform Commission & Escrow Settings</h2>
              <p className="text-xs text-slate-500">Configure global transaction fees and automated payout parameters.</p>
            </div>
          </div>

          {savedCommissionNotice && (
            <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 font-medium flex items-center space-x-2">
          ...
          <form onSubmit={handleSaveCommission} className="space-y-5">
            ...
            <div className="pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-3 gap-3">
                {['12 Hours', '24 Hours (Default)', '48 Hours'].map((opt, i) => (
                  <label key={opt} className={`p-3 rounded-xl border text-center cursor-pointer transition text-xs font-semibold ${
            ...
            <div className="pt-6">
              <button
                type="submit"
                className="px-6 py-2.5 bg-[#1B3A4B] hover:bg-[#132a36] text-white font-semibold text-xs rounded-xl shadow-xs transition flex items-center space-x-2"
              >
        ...
        >
          <div className="mt-4 mb-2 p-4 bg-slate-50 rounded-xl border border-slate-200">
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-2 text-xs font-bold text-slate-400">৳ BDT</span>
            </div>
