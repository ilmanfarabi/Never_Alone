    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
          ...
          >
            ...
            {activeSOSCount > 0 && (
              <span className="bg-white text-red-600 text-[10px] px-1.5 py-0.2 rounded-full font-black">
                {activeSOSCount}
              </span>
            )}
          </button>
        ...
        <div className="space-y-6">
          ...
          {activeSOSAlerts.length > 0 ? (
            <div className="bg-red-50 border-2 border-red-500/80 rounded-2xl p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-red-700 hover:bg-red-800 text-white text-xs font-bold rounded-lg transition shadow-xs"
                >
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <h3 className="text-base font-black text-slate-900 mt-1">
                          {alert.userName} <span className="text-xs font-normal text-slate-500 capitalize">({alert.triggeredBy})</span>
                        </h3>
                        <p className="text-xs text-slate-500">Booking Reference: <span className="font-mono text-indigo-600 font-semibold">{alert.bookingId}</span></p>
                      </div>

                      <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded-lg flex items-center">
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 text-center space-y-3">
              ...
              <div>
                <h2 className="text-base font-bold text-emerald-950">Zero Active Emergency Distresses</h2>
                <p className="text-xs text-emerald-700 mt-0.5">All ongoing bookings are in safe status without distress pings.</p>
              </div>
              <button
                onClick={triggerMockSOS}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
            <h2 className="text-base font-bold text-slate-900 mb-4">Historical SOS Incident Audit</h2>
            <div className="space-y-3">
              {sosAlerts.filter((s: SOSAlert) => s.status === 'resolved' || s.status === 'false_alarm').map((alert: SOSAlert) => (
                <div key={alert.id} className="p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row md:items-center md:justify-between gap-3 text-xs">
                  <div>
                    <div className="flex items-center space-x-2">
      ...
      {activeTab === 'reports' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-6">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Incident & Policy Violation Reports ({safetyReports.length})</h2>
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                  <div className="space-y-2 max-w-3xl">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs text-slate-400 font-mono">Report #{report.id}</span>
                    </div>
                    ...
                    <div className="text-sm">
                      ...
                      {report.bookingId && (
                        <span className="text-xs text-slate-500 ml-2">
                          (Booking <span className="font-mono text-indigo-600 font-semibold">{report.bookingId}</span>)
                        </span>
                      )}
                    </div>
                    ...
                    {report.evidenceUrls && report.evidenceUrls.length > 0 && (
                      <div className="flex items-center space-x-2 pt-1">
                        <span className="text-[11px] font-bold text-slate-500 uppercase">Attached Evidence:</span>
                        {report.evidenceUrls.map((url: string, idx: number) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center space-x-1 px-2 py-0.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-xs font-semibold"
                          >
                    ...
                    {report.status !== 'dismissed' && report.status !== 'resolved' && (
                      <div className="flex flex-wrap lg:flex-col gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-lg text-xs font-semibold transition"
                        >
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-800 border border-orange-300 rounded-lg text-xs font-semibold transition"
                        >
                          Suspend 14 Days
                        </button>
      ...
      {activeTab === 'chat' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-6">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Proactive Chat Moderation Queue ({chatFlags.length})</h2>
            <p className="text-xs text-slate-500">
            ...
            {chatFlags.map((flag: ChatFlag) => (
              <div key={flag.id} className="p-5 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 transition space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center space-x-2">
                    <span className="px-2.5 py-0.5 rounded text-[11px] font-black bg-red-100 text-red-800">
                      FLAGGED: "{flag.detectedKeywords.join(', ')}"
                    </span>
                    <StatusBadge status={flag.status} />
                    <span className="text-xs text-slate-400 font-mono">Chat #{flag.id}</span>
                  </div>
                ...
                {/* Message snippet */}
                <div className="p-3 bg-red-50/50 rounded-xl border border-red-100 text-xs text-slate-900">
